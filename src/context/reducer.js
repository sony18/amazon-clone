...
const updateBasketItem = (state, item) => {
     ...
     let firstIndex = updatedBasket.findIndex(
...
const loadBasketItems = (state, action) => {
     ...
     let itemToLoad = tempBasket?.find(
...
export const selectBasketTotalAmount = (basket) =>
     basket?.reduce(
          (total, basketitem) => (total += parseFloat(basketitem.amount)),
          0
     );
export const selectBasketTotalItem = (basket) =>
     basket?.reduce((totalItem, item) => (totalItem += item.quantity), 0);
