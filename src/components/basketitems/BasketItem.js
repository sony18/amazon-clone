...
const BasketItem = ({ item: { id, name, img, price, description, quantity,rating } }) => {
    ...
    return (
        <div className="basketitem">
            ...
            <div className="basketitem__description">
                <Link to="/basket"><span>{name} </span> {description}</Link>
                <span>{Array(rating).fill().map(_ => <span>⭐</span>)}</span>
            ...
            <p className="basketitem__price"><span className="basketitem__unit">£</span>{Number(price).toFixed(2)}</p>
        </div>
    )
}
